...
│   ├── prompt.py            # 各阶段 prompt 构建与解析
│   ├── config.py            # 从项目根加载 .env
│   ├── types.py             # Tool / EvaluationResult
│   ├── llm/                 # create_deepseek_llm（OpenAI 兼容）
│   └── tools/               # calculator / word_count / get_current_time
├── pyproject.toml
├── uv.lock
├── .python-version
└── .env.example
...
依赖由 `pyproject.toml` + `uv.lock` 管理（uv 项目，要求 Python ≥ 3.13）。
...
for trial in range(1, max_trials + 1):
    output = run_action(task, reflections, tools, llm)              # agent/action.py
    result = evaluate(task, output, llm)                            # agent/evaluator.py
    if result.success:
        return output
    reflection = reflect(task, output, result.feedback,
                         result.score, reflections, llm)            # agent/reflector.py
    reflections = trim_reflections(reflections + [reflection])
```
...
`agent/config.py` 会从项目根目录（即 `Reflexion/`）读取 `.env`，因此 `.env` 放在本目录即可。
...
**运行示例时序图**（`uv run main.py`，任务：计算 21+21 并统计答案串字符数）
